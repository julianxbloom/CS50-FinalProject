...
The app is programmed using Python's [Flask](https://flask.palletsprojects.com/en/) library for the server side, and HTML, CSS and Javascript for the client side. To that extent, the project has to fit with Flask's required file architecture: a app.py file, containing the several routes users can go througth when using the app, a static folder, containing all icons displayed in the app and a templates folder, where the html templates for each route are stored. In add to these files, I separated from functions and variables from app.py, and put them in helpers.py. It helps lighten the main file of the webapp, insomuch as the list containing localities is really long. 
